...
class ModelArguments:
    ...
    tune_llm_utill_layer: Optional[int] = field(default=-1)
...
class DataArguments:
    ...
    data_scale: Optional[int] = field(default=-1)
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        if data_args.data_scale != -1:
            list_data_dict = list_data_dict[:data_args.data_scale]
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        while attempt < max_attempt:
            try:
                ...
                attempt += 1
                print(self.list_data_dict[i]['image'])
    ...
    def _sample_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            # Some filename correction in MGM's data
            ...
            elif 'chartqa' in image_file:
                image_file = image_file.replace('images', 'png')
...
def unlock_vit(training_args, model_args, vision_tower):
    ...
    if model_args.tune_vit_from_layer != -1:
        ...
        rank0_print(f'Tune the all layers of the vision tower!')
...
def unlock_llm(training_args, model_args, model):
    if model_args.tune_llm_utill_layer == -2:
        for n, p in model.named_parameters():
            if n.startswith('model.layers') or n.startswith('model.embed_tokens') \
                or n.startswith('model.norm') or 'lm_head' in n:
                p.requires_grad = False
        return
    rank0_print(f'Tune the language model! LR for LLM is {training_args.learning_rate}.')
    assert model_args.tune_llm_utill_layer <= model.config.num_hidden_layers

    if model_args.tune_llm_utill_layer != -1:
        rank0_print(f'Tune the language model utill layer {model_args.tune_llm_utill_layer}!')
    else:
        rank0_print(f'Tune the all layers of the language model!')

    for n, p in model.named_parameters():
        if not n.startswith('model.layers'):
            if 'model.embed_tokens' in n:
                p.requires_grad = True
            if 'model.norm' in n or 'lm_head' in n:
                if model_args.tune_llm_utill_layer == model.config.num_hidden_layers or \
                    model_args.tune_llm_utill_layer == -1:
                    p.requires_grad = True
                else:
                    p.requires_grad = False
            continue
        if model_args.tune_llm_utill_layer != -1:
            ...
            if layer_id <= model_args.tune_llm_utill_layer:
                p.requires_grad = True
            else:
                p.requires_grad = False
        else:
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        model.config.tune_language_model = training_args.tune_language_model = model_args.tune_language_model
        model.config.tune_entire_model = training_args.tune_entire_model = model_args.tune_entire_model

        # if tuning the entire model
        if model_args.tune_entire_model:
            if training_args.lora_enable:
                raise NotImplementedError("LoRA tuning can not be implemented when 'tune_entire_model=True'!")

            model.requires_grad_(True)
            lr_of_vit = training_args.vision_tower_lr if training_args.vision_tower_lr is not None \
                and training_args.vision_tower_lr != 0 else training_args.learning_rate
            lr_of_mlp = training_args.mm_projector_lr if training_args.mm_projector_lr is not None \
                and training_args.mm_projector_lr != 0 else training_args.learning_rate
            rank0_print(f'Tune entire model!')
            rank0_print(f'Tune the ViT! The LR of ViT is {lr_of_vit}')
            rank0_print(f'Tune the MLP! The LR of MLP is {lr_of_mlp}')
            rank0_print(f'Tune the LLM! The LR of LLM is {training_args.learning_rate}')
        else:
            # if tuning selected layers of vision encoder
            if model_args.tune_vision_tower:
                unlock_vit(training_args, model_args, vision_tower)

            # if tuning selected layers of language model
            if model_args.tune_language_model:
                unlock_llm(training_args, model_args, model)
    ...
    class SkipLastStepsCallback(TrainerCallback):
        ...
        def on_step_end(self, args, state, control: TrainerControl, **kwargs):
            if state.global_step >= self.stop_step:
                print("============state.global_step:",state.global_step)
                control.should_training_stop = True

    skip_callback = SkipLastStepsCallback(stop_step = 5195)
    # skip_callback = SkipLastStepsCallback(stop_step = 5)

    # training_args.gradient_checkpointing_kwargs = {'use_reentrant':False}
    model.config.vocab_weight = training_args.vocab_weight
    model.config.vocab_loss_type = training_args.vocab_loss_type

    trainer = LLaVATrainer(model=model,
                    tokenizer=tokenizer,
                    args=training_args,
                    # callbacks=[skip_callback],
                    **data_module)
    ...
    if training_args.lora_enable and not training_args.lora_merge_weights:
    ...
    elif training_args.lora_enable and training_args.lora_merge_weights:
        if trainer.deepspeed:
            torch.cuda.synchronize()
